...
function create(req, res) {
  ...
  if (!errors.isEmpty()) {
    ...
    const appoint = Appointment.findOne({ date: req.body.date, time: req.body.time, themeType: req.body.themeType }).exec()
    appoint
          ...
          Appointment
            ...
            .then(async function(appointment) {
              try {
                ...
                const promise2 = User.findOneAndUpdate({ username: req.body.themeType }, { $push: { appointment: appointment } }, { new: true })
...
function themeAppointment(req, res) {
  Appointment
    .find({ themeType: req.params.name })
    .then(appointment => {
      return res.status(200).json(appointment)
    })
}
...
function remove(req, res) {
  ...
  allUser
    .then(async user => {
      ...
      try {
        const promise1 = User.findOneAndUpdate({ appointment: req.params.id, role: 'admin' }, { $pull: { appointment: req.params.id } }, { new: true })
        const promise2 = User.findOneAndUpdate({ appointment: req.params.id, role: 'user2Normal' }, { $pull: { appointment: req.params.id } }, { new: true })
        const result = await Promise.all([promise1, promise2])
