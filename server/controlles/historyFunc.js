  ...
  if (!errors.isEmpty()) {
    ...
    History.create(req.body)
      .then(history => {
        ...
        return User.findOneAndUpdate({ username: req.body.theme }, { $push: { history: history } }, { new: true })
      })
      .then(history => {
...
function themeHistory(req, res) {
  History
  // patient will only see their own histories
    .find({ theme: req.currentUser.username })
    .then(history => {
      return res.status(200).json(history)
    })
}
...
function remove(req, res) {
  History
    .findById(req.params.id)
    .then(history => {
      if (!history) return res.status(404).json({ message: 'theme history not found' })
